...
function CitySearch() {
  ...
  const handleAddToCart = async (itemName, itemPrice, quantity) => {
    ...
    const response = await fetch("https://green-grocer-app.onrender.com/api/add-to-cart", {
      method: "POST",
      headers: {
  ...
  useEffect(() => {
    axios
      .get("https://green-grocer-app.onrender.com/api/cities")
      .then((response) => {
  ...
  const handleSearch = () => {
    ...
    axios
      .get(`https://green-grocer-app.onrender.com/api/city/${selectedCity}`)
      .then((response) => {
